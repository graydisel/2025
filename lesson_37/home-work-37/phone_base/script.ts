...
console.log('#1');
const filterBySmallBalance = function (users: User[]): string[] {
   return users
       .filter((user: User) => parseFloat(user.balance.replace(/[$,]/g, '')) < 2000)
       .map((user: User) => user.phone);
}
console.log(filterBySmallBalance(users));
// // #2 Знайти суму всіх балансів користувачів
console.log('#2');
const sumUsersBalance = function (users: User[]): string {
  return users.reduce((accumulator: number, currentUser: User) => accumulator + parseFloat(currentUser.balance.replace(/[$,]/g, '')), 0)
      .toFixed(2);
}
console.log(sumUsersBalance(users));
// // #3 Знайти користувача з максімальним балансом, вивести його
console.log('#3');
const maxBalanceUser = function (users: User[]): number {
  let maxUser: number = 0;
  users.forEach((user: User): void => {
    if (parseFloat(user.balance.replace(/[$,]/g, '')) > maxUser) {
    maxUser = parseFloat(user.balance.replace(/[$,]/g, ''));
  }});
  return maxUser;
}
...
console.log('#4');
const repeatUsersName = function (users: User[]) {
  const nameCounts: Record<string, number> = users.reduce((acc: Record<string, number>, currentUser): object => {
    acc[currentUser.name] = (acc[currentUser.name] || 0) + 1;
    return acc;
  }, {});
